...
select staff_id, count(*) as 'Cuenta por empleado' from rental
group by staff_id;

/*4. Using the film table, find out how many films were released each year.*/

select release_year, count(*) as 'Films per Year' from film
...
select rating, count(*) as 'Rating per Film' from film
group by rating;

/*6. What is the mean length of the film for each rating type. Round off the average lengths to two decimal places*/

select rating, round(avg(length),2) as 'Rating per Film' from film
group by rating;
